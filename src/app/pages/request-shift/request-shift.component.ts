...
export class RequestShiftComponent implements OnInit {
  ...
  loadSpecialist() {
    ...
    this.availableTurn = [];
    let t = [];
    this.userService.getUserAllSpecialist().subscribe(A=>{
        t = A,
        t.forEach(n=>{
            var a;
            null === (a = n.specialty) || void 0 === a || Array.isArray(a) && a.forEach(i => {
              ("" == this.specialtySelected || i.name == this.specialtySelected) && this.specialist.push(n)
          })
