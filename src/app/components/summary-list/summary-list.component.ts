...
export class SummaryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userService.getSumariId(this.user.uid!, 'patientUid').subscribe((users) => {
      ...
      users.forEach(res => {
        if (!this.listSpecialty.find(srch => srch == res.specialist)) {
          this.listSpecialty.push(res.specialist!);
        }
      })
    })
  }
  ...
  downloadForSpecialty(specialist: string) {
    let newList = this.listSummary.filter(fill => fill.specialist
       == specialist);
